package me.andreraimundo.belarosa_backend.dto.mercadopago;

import java.io.Serializable;

import javax.persistence.Entity;
import javax.persistence.GeneratedValue;
import javax.persistence.GenerationType;
import javax.persistence.Id;
import javax.persistence.JoinColumn;
import javax.persistence.OneToOne;

import com.fasterxml.jackson.annotation.JsonIgnore;

@Entity
public class PixApplicationData  implements Serializable {

    private static final long serialVersionUID = 1L;

    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Integer id;

    private String name;
    private String version;

    @JsonIgnore
    @OneToOne
    @JoinColumn(name="pixPointOfInteraction_id")
    private PixPointOfInteraction pixPointOfInteraction;

    public PixApplicationData() {

    }

    public PixApplicationData(Integer id, String name, String version, PixPointOfInteraction pixPointOfInteraction) {
        this.id = id;
        this.name = name;
        this.version = version;
        this.pixPointOfInteraction = pixPointOfInteraction;
    }

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getVersion() {
        return version;
    }

    public void setVersion(String version) {
        this.version = version;
    }

    public PixPointOfInteraction getPixPointOfInteraction() {
        return pixPointOfInteraction;
    }

    public void setPixPointOfInteraction(PixPointOfInteraction pixPointOfInteraction) {
        this.pixPointOfInteraction = pixPointOfInteraction;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + ((id == null) ? 0 : id.hashCode());
        return result;
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj)
            return true;
        if (obj == null)
            return false;
        if (getClass() != obj.getClass())
            return false;
        PixApplicationData other = (PixApplicationData) obj;
        if (id == null) {
            if (other.id != null)
                return false;
        } else if (!id.equals(other.id))
            return false;
        return true;
    }
    
}
